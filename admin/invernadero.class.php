<?php
 ...
 class especialidad extends sistema {
    ...
    function update ($id, $data){
        $this->conexion();
        $result = [];
        $sql = 'update egresado set no_control=:no_control, nombre_completo=:nombre_completo, especialidad=:especialidad, nombre_proyecto=:nombre_proyecto, opc_titulacion=:opc_titulacion, status=:status, fecha_examen=:fecha_examen, asesor=:asesor, sinodal1=:sinodal1, sinodal2=:sinodal2, sinodal3=:sinodal3 where no_control=:no_control;';
        ...
        $modificar->bindParam(':no_control',$data['no_control'], PDO::PARAM_INT);
    ...
    function readOne ($id){
        $this->conexion();
        $result = [];
        $consulta = 'SELECT * FROM especialidad where id_especialidad=:id_especialidad;';
